...
- ~~DoD box 5 ("every landed task") and `BENCHMARKS §2` ("one row per task T0…T11") specify different row sets with nothing in either doc reconciling them~~ — **RESOLVED 19 Aug, Pooya's call**: `BENCHMARKS §2` now says *one row per landed task* and carries no range, because the range was stale rather than principled — it was correct on the day it was typed and never updated. `T0…T11` was also the bound that put T15 (the deploy that shipped `localhost` product URLs to every card) outside the judged set, which is precisely the task a *faithful/honest* axis exists to catch.
...
**What changed:** A live model now does intake only — free text to constraints — behind `POST /v1/chat` on the platform origin, with the FSM keeping retrieval, the obstacle and the chip row. Every failure returns a bodiless 503 and the widget answers from the local brain, so the demo survives with no key.

**Complaints**
- Tree was dirty with two unrelated batches (T9 round 2, a Vercel deploy batch) and mtimes showed another desk editing live at pickup; Pooya was asked and chose the tangle, then that work landed on its own before building started — harm: a check-in cycle spent on a state that resolved itself before the answer was needed; risk: the same shared-tree hazard is already logged against T7, T9, T10 and T15 in this session and is still unaddressed at the process level.
- No provider API key existed anywhere in the repo going into a task titled "real LLM turn"; one had to be sourced from a sibling repo's `.env.local`, and a `.gitignore` rule had to land before it could be written down safely — harm: an unplanned credential detour before any T13 code could run; risk: recurs for the next task that assumes a runtime secret is already provisioned.
- The DoD grades the fallback, not the feature: boxes 3, 4, 5, 6 and 7 are already true on HEAD, so an endpoint that 503s on every turn passes most of the checklist — harm: the acceptance criteria could not distinguish "built" from "not built" until the reviewer invented that distinction; risk: recurs for any "degrade, never break" task where nobody writes a box only the live path can satisfy.
- Box 2's own worked QA example ("nothing too sweet", "won't upset my stomach") shares keywords with `parse.ts`'s table, failing the box's own "shares no keyword" test — harm: the task's stated acceptance example disqualified itself, caught only by adversarial review; risk: recurs whenever a QA example is written without checking it against the constraint it demonstrates.
- Box 7 ("gzip unchanged") is unsatisfiable beside the task's own scope line, which mandates the widget call a new endpoint — harm: renegotiated mid-task to "reported, not claimed"; risk: recurs whenever DoD boxes are not checked against each other, and against the scope, before building.
- The plan asserted `packages/agent/biome.json` already kept `ai`/`zod` out of the bundle; it restricted `apps/**` only and the rule had to be written from scratch — harm: a false premise about one specific file caught by review rather than by opening it; risk: this is the THIRD time an agent has made an unverified claim about this exact rule (the T5/T6 and T7 rounds both corrected earlier ones) — it keeps being asserted about instead of read.
- `bun run lint` went red twice for reasons that were not this task: another desk's in-repo git worktree shadowed `biome.json` and broke lint repo-wide, then an unformatted `tools/build-velde.ts` failed the same gate — harm: two false-red gate runs had to be diagnosed and attributed away from T13 before the real result could be trusted; risk: a shared tree's gate keeps breaking on files no single desk owns.
- Another desk rewrote `DECISIONS.md` mid-task (T10's trim, 1444→814 words) and deleted the section T13 had just added — harm: finished work was silently destroyed and rewritten against someone else's word budget; risk: the second time a peer has overwritten `DECISIONS.md` during another task's active edit, and nothing changed after the first.
- The initial build shipped a demo-killing bug: `live` latched off permanently on any failure while client and server timeouts were both 5s, so the client always won the race and one slow turn switched the model off for the rest of the session — harm: precisely the hotel-wifi scenario the scope text exists to prevent, shipped in the first pass and caught only by the diff review; risk: two independently declared timeout constants that happen to be equal is easy to reintroduce when either changes alone.
- A shipped comment described a tool self-correction mechanism that does not exist — `ai@7` defaults `stopWhen` to one step, so the tool result never reaches the model — harm: fabricated documentation of unimplemented behaviour would have shipped, alongside dead compute on every paid turn; risk: the same class as T11's invented contrast ratio, i.e. a repeat pattern of confidently describing behaviour nobody verified.
- `mergeChips` dedupes on chip `id`, so KRACHT's co-extensive `protein`/`protein-shake` tags could seat two undroppable chips for one constraint, breaking the graded "restorable in one tap" — harm: shipped in the initial build with no test that could have caught it; risk: any future catalog tag overlapping an existing one reintroduces it, since nothing enforces tag exclusivity structurally.
- Zero runnable checks shipped with the initial build of a task touching a network path, tool calling and a prompt-injection surface; 14 tests were added only after the diff review forced it, and the DoD's own "stays offline" requirement had nothing asserting it until then — harm: two real defects reached a review pass instead of a failing test; risk: recurs whenever "build first, get reviewed" substitutes for gating tests inside the build step.
