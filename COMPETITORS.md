...
| — | — | The constant, non-overridable AI signature (EU AI Act Art. 50) | ✅ T7 — **built, and now safe to point at.** On the launcher, not the panel footer, because Art. 50 says "at the latest at the time of the first interaction" and a footer is seen only after the shopper has already opened and typed. Painted on an `AA_GUARANTEED_PAIRS` pair so it stays legible under every config the engine can emit *(was: decided-not-built, 2026-08-19 [T10])* |
...
1. **"We're the only one that guarantees legibility, and we can prove it on 1400 pairs."** Eight products checked, zero documented contrast clamps; ours is enforced in `derive.ts` and measured by `bun bench contrast` — 200 seeded configs, 1400 pairs, worst ratio 4.500:1. The *merchant-facing* half is now **built (T7)**, in an amended shape: a literal before/after of every clamped pair degenerates to a constant (`textPrimary` moves in 0 of 4 real brands, `textOnAccent` in 0/2000), so the panel reports the 7 pairs' shipped ratios, `textMuted`'s genuine move, any pair that fell back *below* the floor, and accent-vs-surface. **The strongest version of this claim on stage is the accent guard**: the engine's guarantee is text-on-background only, so a near-white accent still yields an invisible launcher — we detect it, name the ratio, and refuse the snippet until it is resolved. Zero of eight competitors do either half. *(Narrowed 2026-08-19 [T10]: the original said "and we show the guarantee", which named a surface no file in the tree renders.)*
...
3. **"The font comes from the merchant's own theme, not from our dropdown."** Five of eight have no font control at all; we take the stylesheet URL their theme already serves and inject it on the host page (`boot.ts`), because `@font-face` cannot resolve inside the shadow root. Accepting a raw `.woff2` — the literal "font as a file" — is now **built (T7)**: the config page takes the file URL and the *platform* wraps it in a generated stylesheet at `/v1/font.css`, so `FontChoice.href` keeps its documented meaning and the shipped widget did not change by a line [ENGINEERING §2.1/§2.2]. *(Narrowed 2026-08-19 [T10]: "as a file" described an upload path the tree does not have.)*
