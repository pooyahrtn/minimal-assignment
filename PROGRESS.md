...
| T15 deploy the three projects | `minimal-assignment` (this desk) | 2026-08-19 10:00, claimed at pickup | New files only: `vercel.json` ×3, `tools/build-velde.ts`, `.vercelignore`. Edits **exactly two bytes of storefront source** — the embed `src=` origin at `apps/shop-velde/render.ts:197` and `apps/shop-kracht/app/layout.tsx:140` [§0 #11, ENGINEERING §1.1 exemption]. **Does not touch** `apps/platform/server.ts`, `packages/tokens/*`, `packages/agent/src/*`, `DECISIONS-LOG.md` mid-build — all hot with the T7 desk right now (verified by mtime, not by asking). |
