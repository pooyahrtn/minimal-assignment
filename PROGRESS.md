...
| T16 six deployed-studio defects (server half) | `studio-server-fixes` worktree (this desk) | 2026-08-19, claimed at pickup | `apps/platform/server.ts` (the `withScheme` helper + the private-host guard), new `apps/platform/withScheme.test.ts`, `TASKS.md`/`PROGRESS.md`/`DECISIONS-LOG.md`. The minted-config read/write and `tools/deploy.sh` were in this row until the KV half was cut; neither is touched. **Does not touch** `apps/platform/ui/*`, `tools/build-platform.ts`, `e2e/*` — the four studio-side defects of the same report landed in the MAIN tree and are uncommitted there. `server.ts` is also T13's file: this desk branched from `2004626`, so T13's uncommitted route is not here and the merge is the seam to watch. |
