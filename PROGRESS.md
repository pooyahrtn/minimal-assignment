...
| e2e gate red (`bun run test:e2e` exits 1) — unowned, no task | `minimal-assignment` (this desk) | 2026-08-19, claimed at pickup | Touches **`e2e/agent.spec.ts` only**. Does **not** touch `bench/*`, `packages/agent/src/*`, `tools/*` — all of which the T9 desk holds. Not the recorded cause ("one KRACHT spec times out under whole-suite CPU contention"): 20 of 20 `agent.spec.ts` tests fail deterministically because the launcher's accessible name gained the Art. 50 disclosure suffix and `launcherOf` still matches `exact: true` against the bare label. |
| T9 hostile-page hardening + polish pass | `minimal-assignment` (this desk) | 2026-08-19 11:20, claimed at pickup | Touches `packages/agent/src/{css,widget}.ts`, `bench/{run,checks}.ts`, `bench/checks/{transcript,divergence}.ts`, new `bench/{browser,overflow}.ts` + `bench/checks/{viewport-375,isolation}.ts`, new `tools/freeze-check.ts`. **Does not touch** `apps/shop-*` (that is the point of the task), `apps/platform/*`, `packages/tokens/*`. Tree was dirty with T7's finished-but-uncommitted close-out at claim time; committed as `cc4466d` first so this task's diff is its own. |
